...
> **АРХИВ с 2026-07-25.** По итогам аудита метод `graph` убран из обоих UI: ни один живой прогон на реальном листе не дал чертежа (см. «Текущий честный статус» ниже — `failed`, `reader_json_invalid`, пустые ответы). Вывод этого же документа — «универсальный VLM fragment больше не координатная основа» — принят. Код остаётся доступным только через API, пока из него не перенесены в `EngineeringDrawingSpec` действительно ценные части: контракт видов (`DrawingGraphView`), устойчивые идентификаторы, evidence и связи `dimension_applies_to`. Действующий план — [`DXF_CAD_DEVELOPMENT_PLAN.md`](./DXF_CAD_DEVELOPMENT_PLAN.md).
...
- [-] Manifest и UI показывают отдельные назначения layout VLM, source-resolution fragment VLM, crop verifier и детерминированного graph-drafter; отдельные detector geometry/relation stages ещё предстоят.
...
- [x] Выполняется `layout VLM → bounded fragment VLM calls → deterministic graph assembly → schema/completeness validation → draft_graph → pixel/dimension/VLM evidence verification`; непрошедший результат не получает статус точного.
...
- [-] VLM layout/view/title-block stage полного листа реализована; detector benchmark и специализированная дообученная модель ещё предстоят.
- [ ] Type-specific source-resolution heads: segment/endpoint/junction, circle/arc, text, dimension, annotation, hatch.
- [-] VLM fragment reader и независимый crop verifier реализованы; live-run подтвердил необходимость отдельного detector geometry и более строгого fragment structured-output gate.
- [-] Детерминированная сборка fragment graph с глобальными координатами, tile-prefixed IDs и непересекающимися ownership-зонами реализована; cross-tile relation solver ещё предстоит.
...
Live evidence staged-reader v3 (один и тот же A3 source SHA-256
`df6b275c17a03e4854984aa98b0df6794882499ac039966838c087b40f2ff915`):

- generation `a29c9bce-4e19-4efb-92b8-fdaf6e2e6576`: Gemma layout дошёл до schema gate, но вернул пустую строку вместо объекта `unresolved_region`; безопасная канонизация принимает только пустые placeholders, содержательные строки остаются ошибкой;
- generation `4dd07220-bb46-48b0-a72c-404917bd22e0`: layout прошёл, fragment на `Qwen3.6 APEX Compact` повторил один размер более 100 раз, использовал чужие поля `bbox_2d/text_content` и оборвал JSON; DXF не создан;
- runtime audit показал, что установленный APEX Compact не объявляет Ollama capability `vision`, хотя registry ошибочно разрешал его в VLM-слотах; capability исправлена, APEX/Qwopus исключены из CAD VLM routing;
- generation `476c236a-5a34-4984-9ea6-bd59f60bba7a`: настоящий multimodal `qwen3.5:9b` увидел layout, но поместил арифметические выражения вместо numeric coordinates; вычисление модельных выражений намеренно запрещено, run остановлен;
- generation `6153e278-5ef9-466a-8c39-fe0a5dd92793`: настоящий `qwen3-vl:32b` выполнял compact layout около пяти минут и вернул пустой response; модель окончательно исключена из coordinate reader;
- текущее воспроизводимое назначение: multimodal `ministral-3:8b` для компактного layout, `qwen3.5:9b` как экспериментальный fail-closed fragment reader и `qwen3-vl:32b` только для независимой проверки маленьких crops; полный `qwen3.6:35b` снят с активного fragment route после Ollama 500;
- generation `1255c5cc-cbf8-446c-9dfa-727f6ad44b71`: `ministral-3:8b` первым быстро вернул валидный layout; полный `qwen3.6:35b` получил Ollama 500 при загрузке, fallback `qwen3.5:9b` повторил `φ51` десятки раз и оборвал первый fragment JSON;
- решение по live evidence: универсальный VLM fragment больше не рассматривается как координатная основа. Следующий обязательный инкремент — geometry/type detector heads с детерминированными координатами плюс отдельные bounded VLM-вызовы только для текста, размеров, символов и семантических связей;
- все перечисленные staged-запуски завершились fail-closed, частичный DXF не создан, `false_exact = 0`; reader promotion по-прежнему не пройден.
