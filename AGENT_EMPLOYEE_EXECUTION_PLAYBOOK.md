...
1. Прочитать `AGENTS.md`, разделы 1–6 этого файла и **только карточку E00**.
...
4. Завершить E00, записать доказательства. Не переходить автоматически к E10,
   новому runtime, sandbox или массовому рефакторингу.
5. Следующая карточка — E01. После E02 провести первую контрольную проверку.
...
### 1.2. Незавершённая работа, которую нельзя принять за готовую

После `548ba327` в рабочем дереве оставлены изменения **без проверки и выкладки**:
...
Это заготовка E00: read-only сравнение текущего AgentTask с квитанцией. Для неё
ещё нет новых тестов и UI, её нельзя объявлять завершённой по старым 179 тестам.
При передаче на другую машину незакоммиченный diff сам не появится: оператор
должен передать его отдельно, либо исполнитель реализует E00 по контракту ниже.
Если дерево уже изменилось, сначала установить, что было сделано после среза.
...
**Статус:** IN_PROGRESS: только непроверенная заготовка. **Зависимость:** `548ba327`.
