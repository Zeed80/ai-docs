...
AI-сотрудник **Света** (OpenClaw agent) обрабатывает счета, письма, чертежи.
...
## Стек
- **Agent**: OpenClaw Gateway v2026.4.x (TS) — AI-сотрудник «Света»
...
- OpenClaw = мозг (planning, reasoning), FastAPI = руки (CRUD, data, async tasks)
- **Degraded mode**: UI работает через REST без OpenClaw
...
openclaw/          — config, prompts, skills, scenarios
...
- Pydantic schemas = единый источник правды для OpenClaw skills (auto-gen YAML)
- 52 skills в 13 категориях, 9 approval gates
- Все endpoints = OpenClaw tools (через `generate-skill-registry.py`)
...
| Вопрос | Кто отвечает |
|---|---|
| «Что делать?» (planning, reasoning) | OpenClaw (gemma4:26b или Claude API) |
| «Как это сделать?» (CRUD, валидация) | FastAPI |
| «Тяжёлая работа» (OCR, PDF, Excel gen) | Celery |
| «Показать пользователю» | Next.js |
| «Можно ли это сделать?» (approval gate) | OpenClaw спрашивает → человек отвечает |
...
52 skills в 13 категориях. Каждый skill = FastAPI endpoint, описанный Pydantic-схемой. Скрипт `generate-skill-registry.py` генерирует YAML для OpenClaw из Pydantic-схем автоматически. Pydantic схемы = единственный источник правды для OpenClaw skills.
