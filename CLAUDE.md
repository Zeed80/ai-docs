...
## Ключевые документы
- `AGENT_EMPLOYEE_ORCHESTRATION.md` — включённый по поручению пользователя режим: главный агент назначает работу более простой модели и независимо принимает её; без ручной передачи карточек пользователем.
- `AGENT_EMPLOYEE_EXECUTION_PLAYBOOK.md` — подробный план передачи реализации другой модели: карточки E00–E52, ограничения, проверки и review gates. E00–E04 прошли реализацию и review; следующий этап E05.1 — адаптеры чтения. Контракт ToolResult v1 проверяется `python3 -m pytest backend/tests/test_tool_result_contract.py -q`.
...
Пилот квитанции получателя: `agent_control.task_propose` сохраняет предложение
и WorkEvent в одной транзакции под блокировкой WorkOrder. Ключ привязан к
владельцу, аргументам и попытке; полномочий не добавляет. `recipient_receipt`
в деталях журнала и UI доказывает прошлую запись в БД, не текущее состояние или
внешнюю доставку; автоматического продолжения нет.
...
- **Dual AI**: конфиденциальные документы — локальный Ollama по умолчанию. Задачи, читающие содержимое (`classification`, `long_context_summarization`, `engineering_reasoning`, OCR, извлечение, чертежи), перечислены в `CONFIDENTIAL_TASKS` и локальны, пока оператор не решит иначе. Planner/auditor и генерация писем — `CLOUD_OPT_IN_TASKS`: тоже локальны по умолчанию. Задача обязана быть в одном из двух списков, за полнотой следит `test_confidential_task_coverage.py`.
  **Не «закрыты жёстко».** `TaskRouting.cloud_override` — именованное исключение: назначение облачной модели на такую задачу через экран моделей записывает решение оператора в маршрут, и роутер признаёт его сильнее, чем `confidential=True` у места вызова. Решение подписано предупреждением «наружу уйдёт само содержимое документов и чертежей», пишется в журнал и видно в манифесте прогона. Формулировка «закрыто жёстко» держалась в этом файле после того, как код перестал так работать (коммит 04a615b), и по ней делались неверные выводы
...
- **Без автоматического запаса**: конвейер работает ТОЛЬКО на модели, которую выбрал оператор. Роутер берёт `models[0]` (или `preferred_model`, если место вызова назвало модель явно) и хвост цепочки не обходит. Решение принято 2026-09-07 по итогам разбора: цепочка давала молчаливую подмену (в настройках одна модель, в работе другая) и целый класс ложных диагнозов — причиной отказа называлась чужая, часто просто нескачанная модель. Хвост по-прежнему ХРАНИТСЯ: слот «Повторное извлечение» (`ocr_large`) читает второй элемент сам и вызывает модель напрямую — это осознанный шаг конвейера, а не замена при сбое
- **Формат ответа**: `backend/app/ai/output_format.py` выбирает механизм принуждения по возможностям модели (схема параметром → принудительный инструмент → json-режим → схема словами), роутер восстанавливает невалидный ответ лестницей repair → переспрос → спуск ступени. Возможности модели подтверждаются живой пробой (`capability_probe.py`), и только подтверждённое несоответствие модальности блокирует назначение
