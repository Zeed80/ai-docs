...
- [-] **A2. Эскизные операции.** Выполнено 2026-07-13: trim, extend (отказывается укорачивать), offset (отрезок/окружность/дуга, сторона вторым кликом), pattern_linear/pattern_polar (`transform.py` + PATCH-ops + инструменты тулбара/командной строки; `SketchOpError`→422); **construction geometry** — флаг `construction` на Entity, исключение из DXF/SVG(/PDF) экспорта, `set_construction`-toggle, рендер пунктиром на канве. Живьём: все 5 операций + extend-422 + construction (из 2 сегм. в DXF 1). Осталось: split/join, blocks/insert.
