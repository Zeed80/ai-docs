...
- [-] **A2. Эскизные операции.** Выполнено 2026-07-13: trim, extend (отказывается укорачивать), offset (отрезок/окружность/дуга, сторона вторым кликом), pattern_linear/pattern_polar, **split** (клик по отрезку делит надвое), **join** (два отрезка с общей вершиной → коллинеарные в один Segment, угол в Polyline), **construction geometry** (флаг `construction`, исключение из DXF/SVG/PDF, toggle, пунктир на канве). Всё в `transform.py` + PATCH-ops + инструменты тулбара/командной строки; `SketchOpError`→422. Живьём: все операции + extend/join-ошибки 422 + construction (из 2 сегм. в DXF 1) + split↔join round-trip. Осталось: **blocks/insert** (переиспользуемые блоки).
