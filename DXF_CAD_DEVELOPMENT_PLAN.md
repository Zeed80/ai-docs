...
- [-] **B4. Corpus и метрики.** Есть `eval_vectorize.py` (cv|neural|arbitrate, DWG ground truth). Добавлено 2026-07-13: **самореферентные метрики качества геометрии** `_geometry_quality` (fragmentation = сегментов/после-консолидации, degenerate/duplicate/open_endpoint rate — прямо меряют «рваность/мусор», БЕЗ ненадёжного выравнивания фото↔DWG), **DXF round-trip rate** (`_dxf_roundtrip`: IR→DXF→ezdxf reopen), агрегаты в summary, **автоматический regression-гейт** `--check-baseline` (exit 1 при просадке recall/coverage-ok/dxf-reopen или росте фрагментации/шума сверх допуска). Тесты test_eval_vectorize_quality (4). Живьём detal_126/cv: frag 1.13, open_endpoint 0.66, dxf_reopen 1.0. Осталось: entity-level P/R требует выровненного GT-корпуса (рендер-пространство); baseline JSON перегенерировать полным прогоном; OCR/dimension accuracy, false-accept rate.
