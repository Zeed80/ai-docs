...
for (const path of SECTIONS) {
  test(`доступность раздела ${path}`, async ({ page, context }) => {
    ...
    const { violations } = await new AxeBuilder({ page })
      ...
      // Контраст пока вне ворот. `text-slate-600` (1.9-2.7:1 на тёмном фоне)
      // исправлен — он был откровенно нечитаем. Но `text-slate-500` даёт
      // 3.7-4.2:1 при норме 4.5 и используется больше восьмисот раз как
      // основной цвет пояснений: это смена вида всего приложения, а не
      // починка дефекта, и решать её владельцу продукта. Пока правило
      // выключено, остальные — critical/serious по разметке, именам и
      // структуре — работают: именно они поймали три раздела, падавших в
      // белый экран на неожиданной форме ответа.
