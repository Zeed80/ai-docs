    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex flex-wrap items-center gap-4 px-6 py-4 bg-zinc-900 border-b border-white/10">
        ...
        <div className="relative ml-auto w-full max-w-md">
          <input
            ...
            className="w-full rounded-lg border border-white/10 bg-zinc-800 px-4 py-2 text-sm text-white placeholder-white/50 focus:border-blue-500/50 focus:outline-none"
          />
        </div>
      </div>
        ...
        <div className="flex flex-1 flex-col overflow-hidden px-6 pt-4">
          ...
          <div className="mt-4 flex-1 overflow-y-auto pb-6">
            {itemLoading && !itemResult ? (
              <div className="py-12 text-sm text-white/50">Поиск…</div>
            ) : (
              <CatalogEntryGrid
                entries={
      ...
      <div className={`px-6 pt-4 ${mode === "items" ? "hidden" : ""}`}>
        <input
          ...
          className="w-full max-w-md px-4 py-2 bg-zinc-800 border border-white/10 text-white placeholder-white/50 rounded-lg text-sm focus:outline-none focus:border-blue-500/50"
        />
      </div>
      ...
      >
        {loading ? (
          <div className="flex items-center gap-2 py-12 text-white/50 text-sm">
            <div className="w-4 h-4 border border-white/30 border-t-white rounded-full animate-spin" />
            Загрузка...
          </div>
        ) : suppliers.length === 0 ? (
          <div className="flex flex-col items-center py-16 text-white/50 gap-2">
            <svg
