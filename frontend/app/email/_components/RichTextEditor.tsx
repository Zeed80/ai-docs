...
export function RichTextEditor({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "min-h-[180px] max-h-[45vh] overflow-auto px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:outline-none prose prose-sm dark:prose-invert max-w-none",
  ...
  if (!editor) {
    return <div className="min-h-[220px] rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900" />;
  }

  const btn = (active: boolean) =>
    `px-2 py-1 text-xs rounded font-medium ${
      active ? "bg-slate-600 text-white" : "text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"
    }`;

  return (
    <div className="rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 overflow-hidden">
      <div className="flex flex-wrap items-center gap-0.5 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-1 py-1">
        ...
        <span className="mx-1 h-4 w-px bg-slate-100 dark:bg-slate-700" />
