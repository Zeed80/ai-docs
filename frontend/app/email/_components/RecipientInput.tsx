...
export function RecipientInput({
  ...
  return (
    <div ref={boxRef} className="relative flex min-w-0 flex-1 flex-wrap items-center gap-1">
      ...
      <div
        className="flex min-h-[30px] flex-1 flex-wrap items-center gap-1 rounded border border-slate-600 bg-slate-700 px-1.5 py-1"
      ...
      >
        {value.map((e) => (
          <span
            ...
            className={`flex items-center gap-1 rounded-full px-1.5 py-0.5 text-xs ${
              EMAIL_RE.test(e) ? "bg-slate-600 text-slate-100" : "bg-red-900/50 text-red-200"
            }`}
            title={e}
          >
            <span
        ...
        <input
          ...
          placeholder={value.length ? "" : "имя или адрес"}
          className="min-w-[120px] flex-1 bg-transparent text-sm text-slate-100 placeholder-slate-500 focus:outline-none"
        />
      </div>

      {open && sug.length > 0 && (
        <div className="absolute left-8 top-full z-30 mt-1 max-h-64 w-[min(420px,90vw)] overflow-auto rounded-lg border border-slate-600 bg-slate-800 py-1 shadow-xl">
          {sug.map((c, i) => (
            <button
              key={c.email}
              ...
              className={`flex w-full items-center gap-2 px-3 py-1.5 text-left ${
                i === active ? "bg-blue-900/40" : "hover:bg-slate-700/50"
              }`}
            >
              <span
              ...
              <span className="min-w-0 flex-1">
                <span className="flex items-center gap-1.5">
                  ...
                  {c.is_favorite && <span className="text-amber-400 text-xs">★</span>}
                </span>
              ...
              <span className="shrink-0 text-[9px] uppercase text-slate-600">
                {c.source === "book"
                  ? "книга"
                  : c.source === "party"
                    ? "поставщик"
