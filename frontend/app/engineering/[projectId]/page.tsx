...
export default function EngineeringProjectPage() {
  ...
  const load = useCallback(async () => {
    try {
      const [detail, materialRows] = await Promise.all([
        engineeringApi.getProject(projectId),
        engineeringApi.listMaterials(),
      ]);
      setProject(detail);
  ...
  useEffect(() => {
    ...
    engineeringApi
      .listAnalysisCases(selectedRevisionId)
      .then(setAnalysisCases)
  ...
  async function createCase(event: FormEvent) {
    ...
    setError(null);
    try {
      const item = await engineeringApi.createAnalysisCase(
        selectedRevision.id,
        {
          name: caseName,
    ...
    <main className="mx-auto w-full max-w-6xl space-y-6 px-4 py-6 md:px-8">
      <header className="flex flex-wrap items-end justify-between gap-3 border-b border-white/10 pb-4">
        <div>
          ...
          {project?.code && (
            <p className="font-mono text-sm text-zinc-400">{project.code}</p>
          )}
        </div>
        <Link
          href="/studio"
      ...
      {error && (
        <div className="border-l-2 border-red-400 bg-red-500/5 px-3 py-2 text-sm text-red-300">
          {error}
        </div>
      )}
        ...
        <div className="border border-white/10">
          ...
          <div className="divide-y divide-white/5">
            {project?.revisions.map((revision: EngineeringRevision) => (
              <button
                ...
                className={`grid w-full grid-cols-[70px_minmax(0,1fr)_120px] gap-3 px-4 py-3 text-left text-sm hover:bg-white/5 ${revision.id === selectedRevisionId ? "bg-sky-500/10" : ""}`}
              >
                <span className="font-mono text-zinc-300">
                  R{revision.revision}
                </span>
                <span className="truncate text-zinc-300">
                  {revision.change_summary || revision.origin}
                </span>
          ...
          >
            <input
              ...
              placeholder="Изменение ревизии"
              className="min-w-0 flex-1 rounded border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-zinc-100"
            />
            <button
              disabled={busy}
        ...
        <section className="border border-white/10">
          ...
          <form onSubmit={createCase} className="grid gap-2 p-4">
            <input
              ...
              disabled={
                !selectedRevision || selectedRevision.status === "approved"
              }
              className="rounded border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 disabled:opacity-50"
            />
            <div className="grid grid-cols-2 gap-2">
              <input
                ...
                className="min-w-0 rounded border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 disabled:opacity-50"
              />
              <input
                value={area}
                type="number"
          ...
          <div className="border-t border-white/10 divide-y divide-white/5">
            {analysisCases.map((item) => (
              <div key={item.id} className="px-4 py-3 text-sm">
                ...
                {item.results.stress_mpa != null && (
                  <p className="mt-1 text-xs text-zinc-400">
                    sigma {item.results.stress_mpa} MPa; запас{" "}
                    {item.results.safety_factor ?? "-"}
                  </p>
                )}
                <button
                  type="button"
