...
type WorkLearning = { status: string; summary?: string; lessons: Record<string, unknown>[];
  provenance: Record<string, unknown>; memory_fact_id?: string; recipe_skill_id?: string;
  extraction_attempts: number; last_error?: Record<string, unknown>; processed_at?: string };
...
export default function WorkOrdersPage() {
  ...
  const loadDetail = useCallback(async (id: string) => {
    const [planResponse, eventResponse, learningResponse] = await Promise.all([
  ...
  async function reprocessLearning() {
    if (!selected) return;
    await api(`/api/work-orders/${selected}/learning/reprocess`, { method: "POST", body: "{}" });
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[360px_1fr] gap-5">
      ...
      <section className="min-w-0">
        {!current ? <div className="h-64 grid place-items-center text-slate-500 border border-dashed border-slate-700 rounded-xl">Выберите поручение</div> : <div className="space-y-4">
          ...
          {learning && <div className="bg-slate-900 border border-slate-700 rounded-xl p-4"><div className="flex items-center justify-between gap-3"><h3 className="font-medium">Память исполнения</h3><div className="flex items-center gap-2"><span className={`text-xs px-2 py-0.5 rounded ${learning.status === "recorded" ? "bg-emerald-900/50 text-emerald-300" : learning.status === "failed" ? "bg-red-950/60 text-red-300" : "bg-violet-950/60 text-violet-300"}`}>{learning.status}</span><button onClick={reprocessLearning} className="px-2 py-1 rounded bg-slate-700 text-xs">Пересобрать</button></div></div>
            {learning.summary && <p className="mt-3 text-sm text-slate-300 whitespace-pre-wrap">{learning.summary}</p>}
            <div className="mt-3 grid grid-cols-1 lg:grid-cols-2 gap-3"><pre className="text-xs bg-slate-950/50 rounded p-3 overflow-auto">{JSON.stringify(learning.lessons, null, 2)}</pre><pre className="text-xs bg-slate-950/50 rounded p-3 overflow-auto">{JSON.stringify(learning.provenance, null, 2)}</pre></div>
            <div className="mt-2 text-[11px] text-slate-500">memory {learning.memory_fact_id || "—"} · recipe {learning.recipe_skill_id || "не создан"} · попыток {learning.extraction_attempts}</div>
            {learning.last_error && <pre className="mt-2 text-xs text-red-300 overflow-auto">{JSON.stringify(learning.last_error, null, 2)}</pre>}
          </div>}
