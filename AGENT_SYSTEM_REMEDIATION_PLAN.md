...
### Б20. Документация — `P3` — `[x]` сделано (2026-08-19)
...
- **Итог**: `docs/work-orders-operator-guide.md` покрывает не только
  `statusClass`'ные статусы (7 из 13 реальных), а ВСЕ 13 статусов
  `WorkOrder` из `WORK_TRANSITIONS` (включая `waiting_external`,
  `received`, `scoping`, `verifying`, `canceled` — без цветной подсветки в
  UI, легко упустить при чтении только `page.tsx`) + все состояния
  `WorkStep`, evidence, approval-блок, force-run debug-кнопку (с явным
  предупреждением «если жмёте регулярно на живых поручениях — сигнал, что
  диспетчер сломан, а не штатный режим»), метрики из Б14, журнал событий.
  Ссылка добавлена в `AGENT_SYSTEM_DEVELOPMENT_PLAN.md` секцию 20 (заодно
  проставлен `[x]` для operator guide, ранее `[ ]`) + перекрёстная ссылка
  на этот файл для будущих сессий.

---

## Итог: весь план закрыт

Все 17 пунктов (A1-A7, Б11-Б12, Б14-Б16, Б18-Б20; Б13 — 4 из 5 подпунктов,
Б17.5 отложен) доведены до `[x]` или явного, задокументированного отказа с
причиной. Два пункта (A6, A7) при проверке оказались уже реализованными —
это отражено как находка, а не молча пропущено. Полная история — в git log
ветки `feat/work-order-learning-and-roadmap`, каждый коммит ссылается на
