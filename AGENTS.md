...
Для кодинга по умолчанию исполнитель `gpt-5.6-terra`, для сложной реализации
и эскалации — `gpt-5.6-sol`, с явным model и `fork_turns=none`.
`gpt-5.6-luna` разрешена только для простых read-only задач: поиск файлов/символов,
сбор ссылок и кратких сведений. Не поручать Luna код, тесты, исправления или review.
При недоступности Sol/Terra не заменять их Luna и не наследовать дорогую модель молча.
Только главный агент
