...
Проект уже вышел за рамки планового workspace: есть FastAPI backend, Next.js UI, Celery, модели документов/счетов/поставщиков, quarantine, approvals, audit, email, export, warehouse/procurement/payment и собственный OpenClaw-like agent loop.

Главный разрыв с `refact.md`: продукт всё ещё частично выглядит как широкий AI Manufacturing Workspace и содержит собственную реализацию агентного слоя. Целевое состояние — узкий, надёжный AI-сотрудник документооборота по счетам/закупкам/складу/финконтролю, где официальный open-source OpenClaw является control plane, а FastAPI остаётся источником истины и единственным исполнителем доменных действий.
...
- Skills registry из OpenAPI и whitelist/approval list в `openclaw/config/gateway.yml`.
...
- `backend/app/ai/agent_loop.py` остаётся кастомным control plane. Его нужно заменить официальным OpenClaw Gateway поэтапно, сохранив whitelist, approval gates, audit и step limits.
...
- P0: официальный OpenClaw migration spike с реальным Gateway, generated tools, deny unknown tools, approval pause/resume и audit каждого tool call.
