...
## 4quater. Реальное развёртывание: что вскрылось (2026-07-25)

Первый прогон на живом стенде упал: `dependency failed to start:
mailcowdockerized-unbound-mailcow-1 is unhealthy`. Разбор и исправления:

1. **Провайдер перехватывает исходящий DNS.** Итеративные запросы к корневым
   серверам возвращают REFUSED (отвечает резолвер провайдера, а не корень),
   ответы для части доменов подменяются. Рекурсивный unbound в такой сети
   нерабочий в принципе. Добавлен `infra/installer/mailcow-dns-forward.sh`:
   детект перехвата → DoT (853), а при его блокировке — sidecar dnscrypt-proxy
   с DoH (443); вызывается из установщика до первого `up`. Конфиги в
   `infra/mailcow/aiw-dns/`, подключение — YAML-мержем в
   `docker-compose.override.yml` (наивная дозапись текстом ломала override,
   в котором уже жила Traefik-часть).
2. **Конфликт IP в сети mailcow.** Первый вариант занял `172.22.1.253`, который
   апстрим резервирует за postfix, — postfix не стартовал с «Address already in
   use». Форвардер переехал на `.247`.
3. **Роут Traefik указывал на порт 80** — подтвердилось на практике (502 на
   вебмейле), после перерендера с `MAILCOW_HTTP_PORT` вебмейл открылся.
4. **Сертификат на почтовых портах** — `mailcow-certdump.sh` отработал: 587 и
   993 локально отдают сертификат Let's Encrypt вместо snakeoil.

Итог: Mailcow развёрнут и работает, DNS-резолв корректный (проверено на живых
MX-запросах), вебмейл доступен по HTTPS. Осталось внешнее: провайдер не
пропускает входящие 25/587/993 и не отдаёт свой PTR — см. разделы 2.3/2.4 в
`mailcow.README` и одноимённые разделы GUI-руководства.
