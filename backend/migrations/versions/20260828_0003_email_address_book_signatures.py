...
def upgrade() -> None:
    ...
    op.add_column(
        "mail_server_config",
        sa.Column("auto_send_max_per_day", sa.Integer(), nullable=False, server_default="20"),
    )
    op.add_column(
        "mail_server_config",
        sa.Column("attachment_retention_days", sa.Integer(), nullable=False, server_default="180"),
    )
...
def downgrade() -> None:
    ...
    if bind.dialect.name == "postgresql":
        for ix in (
            "ix_email_contacts_name_trgm",
            "ix_email_contacts_email_trgm",
            "ix_email_messages_body_trgm",
            "ix_email_messages_subject_trgm",
        ):
