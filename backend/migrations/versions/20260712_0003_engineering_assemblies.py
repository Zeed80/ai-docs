...
def upgrade() -> None:
    ...
    if "engineering_assemblies" not in tables:
        op.create_table(
            ...
            sa.Column(
                ...
                GUID(),
                sa.ForeignKey("engineering_revisions.id"),
                nullable=False,
            ),
            ...
            sa.UniqueConstraint(
                "engineering_revision_id", "name", name="uq_engineering_assembly_name"
            ),
        )
        op.create_index(
            "ix_engineering_assemblies_engineering_revision_id",
            "engineering_assemblies",
            ["engineering_revision_id"],
        )
    if "engineering_assembly_components" not in tables:
        op.create_table(
            ...
            sa.Column(
                "engineering_assembly_id",
                GUID(),
                sa.ForeignKey("engineering_assemblies.id"),
                nullable=False,
            ),
            sa.Column("component_revision_id", GUID(), sa.ForeignKey("engineering_revisions.id")),
            sa.Column("instance_key", sa.String(160), nullable=False),
        ...
        op.create_index(
            "ix_engineering_assembly_components_engineering_assembly_id",
            "engineering_assembly_components",
            ["engineering_assembly_id"],
        )
        op.create_index(
            "ix_engineering_assembly_components_component_revision_id",
            "engineering_assembly_components",
    ...
    if "engineering_assembly_mates" not in tables:
        op.create_table(
            ...
            sa.Column("mate_type", sa.String(40), nullable=False),
            sa.Column("first_instance_key", sa.String(160), nullable=False),
            sa.Column("second_instance_key", sa.String(160), nullable=False),
            sa.Column("parameters", sa.JSON(), nullable=False, server_default="{}"),
            sa.Column("status", sa.String(30), nullable=False, server_default="valid"),
        )
        op.create_index(
            "ix_engineering_assembly_mates_engineering_assembly_id",
            "engineering_assembly_mates",
            ["engineering_assembly_id"],
        )
