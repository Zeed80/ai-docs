...
def upgrade() -> None:
    ...
    if "kind" not in bom_columns:
        op.add_column(
            "boms", sa.Column("kind", sa.String(10), nullable=False, server_default="ebom")
        )
    if "source_bom_id" not in bom_columns:
        op.add_column(
            "boms", sa.Column("source_bom_id", GUID(), sa.ForeignKey("boms.id"), nullable=True)
        )
    ...
    if "substitutes" not in line_columns:
        op.add_column(
            "bom_lines", sa.Column("substitutes", sa.JSON(), nullable=False, server_default="[]")
        )
