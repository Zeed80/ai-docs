        ...
        if spec.fit_system and spec.machining_stage == "rough":
            issues.append(
                _make_check(
                    ...
                    plan_id,
                    evidence={
                        "fit_system": spec.fit_system,
                        "machining_stage": spec.machining_stage,
                    },
                )
            )
...
def run_normcontrol(plan_id: uuid.UUID, db: Session) -> dict[str, Any]:
    ...
    surface_specs: list[SurfaceMachiningSpec] = (
        db.query(SurfaceMachiningSpec).filter(SurfaceMachiningSpec.process_plan_id == plan_id).all()
    )
    ...
    plan.normcontrol_checked_at = datetime.now(UTC)
