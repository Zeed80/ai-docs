        ...
        if diameter <= STANDARD_DRILLS_MM[-1] and not any(
            ...
            findings.append(
                DfmFinding(
                    ...
                    entity_ids=[circle.id],
                    evidence={"diameter_mm": diameter, "nearest_standard_mm": nearest},
                )
            )
...
def _check_internal_radii(ir: CadIR, scale: float) -> list[DfmFinding]:
    ...
    for arc in (e for e in ir.entities if isinstance(e, Arc) and not e.construction):
        ...
        if radius < MIN_TOOL_RADIUS_MM:
            findings.append(
                DfmFinding(
                    code="DFM_SMALL_INTERNAL_RADIUS",
                    severity="warn",
...
def _check_threads(ir: CadIR) -> list[DfmFinding]:
    ...
    for entity in ir.entities:
        if (
            getattr(entity, "type", None) != "annotation"
            or getattr(entity, "kind", None) != "thread"
        ):
            continue
            ...
            if abs(pitch - coarse) > 1e-9 and pitch > coarse:
                findings.append(
                    DfmFinding(
                        ...
                        entity_ids=[entity.id],
                        evidence={"pitch_mm": pitch, "coarse_mm": coarse},
                    )
                )
    return findings
