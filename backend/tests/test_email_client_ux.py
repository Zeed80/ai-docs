...
async def box(db_session):
    cfg = MailboxConfig(
        name="ux-box", imap_host="imap.example.com", imap_port=993,
        imap_user="ux", imap_password_encrypted="x", imap_ssl=True,
        smtp_host="smtp.example.com", smtp_port=587, smtp_user="ux@example.com",
...
async def test_draft_returns_attachment_names_not_only_ids(
    client: AsyncClient, db_session, box
):
    ...
    att = EmailAttachment(
        filename="счёт-2026-04.pdf", content_type="application/pdf",
        size=12345, sha256="a" * 64, storage_path="x/y", uploaded_by_sub="dev-user",
    )
    db_session.add(att)
    await db_session.commit()
...
async def test_draft_keeps_the_conversation_it_replies_to(
    client: AsyncClient, db_session, box
):
    ...
    msg = EmailMessage(
        thread=thread, mailbox="ux-box", subject="Счёт",
        from_address="supplier@example.com", to_addresses=["ux@example.com"],
        body_text="текст", received_at=datetime.now(timezone.utc),
        message_id_header=f"<{uuid.uuid4()}@example.com>", is_inbound=True,
    )
    db_session.add_all([thread, msg])
    await db_session.commit()

    draft = await _draft(
        client, thread_id=str(thread.id), in_reply_to_message_id=str(msg.id)
    )
...
async def test_drafts_can_be_scoped_to_the_open_mailbox(
    client: AsyncClient, db_session, box
):
    """Папка «Черновики» показывала всё скопом, независимо от выбранного ящика."""
    db_session.add(MailboxConfig(
        name="other-box", imap_host="imap.example.com", imap_port=993,
        imap_user="other", imap_password_encrypted="x", imap_ssl=True,
        smtp_host="smtp.example.com", smtp_port=587, smtp_user="other@example.com",
        smtp_password_encrypted="y", is_active=True,
    ))
    await db_session.commit()
...
async def test_sent_folder_shows_the_recipient_not_ourselves(
    client: AsyncClient, db_session, box
):
    """В «Отправленных» sender — это мы, и список выглядел как переписка с
    собой: собеседника там нужно брать из получателей."""
    thread = EmailThread(
        subject="Запрос КП", mailbox="ux-box", message_count=1, folder="sent",
    )
    db_session.add_all([
        thread,
        EmailMessage(
            thread=thread, mailbox="ux-box", subject="Запрос КП",
            from_address="ux@example.com", to_addresses=["sales@romex.example"],
            body_text="текст", sent_at=datetime.now(timezone.utc),
            received_at=datetime.now(timezone.utc),
            message_id_header=f"<{uuid.uuid4()}@example.com>", is_inbound=False,
            folder="sent",
        ),
    ])
    await db_session.commit()
