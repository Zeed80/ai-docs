...
# ── Невозможный паз не уходит в ядро ────────────────────────────────────────


def test_a_keyway_across_a_step_is_not_sent_to_the_kernel():
    """Замерено на живом ядре, а не выведено из общих соображений.

    Паз шириной 8 мм ВНУТРИ одной ступени строится. Он же через уступ Ø35/Ø30
    даёт `brep_valid: False` и HTTP 500 на всю сборку — тело детали не
    получается вовсе. Узкий паз (3,5 мм) через тот же уступ проходил только
    потому, что не успевал развалить оболочку: тело собиралось с молча
    неверным вырезом.

    Поэтому такой паз пропускается, а не роняет прогон: остальная деталь
    прочитана верно, а маркер «— не построен» отдаёт паз в excluded_geometry,
    где человек его увидит.
    """
    from app.ai.cad_solid import _cut_features

    missing: list[str] = []
    # 46..68 при ступени Ø30 54..78 — ровно живой случай.
    body = {
        "keyways": [{"axial_start_mm": 46.0, "length_mm": 22.0, "width_mm": 8.0, "depth_mm": 4.0}]
    }
    features = _cut_features(body, OUTER, missing)

    assert not [item for item in features if item.kind == "keyway"]
    assert any("пересекает уступ" in item and "не построен" in item for item in missing)


def test_a_keyway_inside_one_step_is_built():
    from app.ai.cad_solid import _cut_features

    missing: list[str] = []
    body = {
        "keyways": [{"axial_start_mm": 58.0, "length_mm": 16.0, "width_mm": 8.0, "depth_mm": 4.0}]
    }
    features = _cut_features(body, OUTER, missing)

    assert [item.kind for item in features if item.kind == "keyway"] == ["keyway"]
    assert not [item for item in missing if "пересекает уступ" in item]


def test_the_skipped_keyway_is_excludable_from_the_preview_not_a_hard_blocker():
    """Иначе пропуск паза заблокировал бы и предпросмотр всей детали."""
    from app.ai.cad_solid import solid_preview_gate

    gate = solid_preview_gate(
        {"blockers": ["шпоночный паз 46..68 мм пересекает уступ Ø35.0 (34..54 мм) — не построен"]}
    )
