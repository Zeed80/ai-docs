...
from app.ai.cad_recognize.spec_fragments import (
    _assign_profile_threads,
    _auxiliary_circular_hole_patterns,
    _callout_numbers,
    _feature_completeness_issues,
    _read_cut_features,
    _recover_external_thread_carrier,
    _resolve_axial_pattern_entry_offset,
    _resolve_axial_pattern_geometry,
    _resolve_axial_pattern_geometry_from_source,
    _resolve_grouped_chamfers,
)
from app.ai.cad_recognize.spec_vectorize import EngineeringDrawingSpec, _coerce_spec_containers
...
def test_removed_sections_preserve_both_unthreaded_hole_families():
    patterns = _auxiliary_circular_hole_patterns(
        {
            "dimensions": [
                {"value": "8 отв. Ø1"},
                {"value": "Ø65"},
...
async def _read(monkeypatch, answer: dict) -> dict:
    ...
    monkeypatch.setattr("app.ai.cad_recognize.spec_fragments._ask", fake_ask)
    return await _read_cut_features(object(), _OUTER, router=object(), confidential=True)
...
async def test_feature_question_is_kept_in_the_parent_reader_audit(monkeypatch):
    ...
    monkeypatch.setattr("app.ai.cad_recognize.spec_fragments._ask", fake_ask)
    await _read_cut_features(object(), _OUTER, router=object(), confidential=True, audit=audit)
...
async def test_the_features_a_real_shaft_has_are_kept(monkeypatch):
    result = await _read(
        monkeypatch,
        {
            ...
            "keyways": [
                {"axial_start_mm": 40.0, "length_mm": 85.0, "width_mm": 12.0, "depth_mm": 5.0}
            ],
            "cross_holes": [
                {"diameter_mm": 9.0, "axial_position_mm": 200.0, "count": 2, "through": True}
            ],
        },
    )
...
async def test_a_cut_deeper_than_the_shaft_is_dropped(monkeypatch):
    result = await _read(
        ...
        {
            "keyways": [
                {"axial_start_mm": 40.0, "length_mm": 85.0, "width_mm": 12.0, "depth_mm": 60.0}
            ],
...
async def test_a_keyway_shorter_than_it_is_wide_is_dropped(monkeypatch):
    result = await _read(
        ...
        {
            "keyways": [
                {"axial_start_mm": 40.0, "length_mm": 5.0, "width_mm": 12.0, "depth_mm": 5.0}
            ],
        },
    )
...
async def test_a_sheet_with_no_such_features_yields_nothing(monkeypatch):
    assert (
        await _read(monkeypatch, {"chamfers": [], "grooves": [], "keyways": [], "cross_holes": []})
        == {}
    )
...
async def test_local_slot_depth_cannot_be_replaced_by_unrelated_sheet_number(monkeypatch):
    ...
    assert [
        (item["length_mm"], item["width_mm"], item["depth_mm"]) for item in result["keyways"]
...
async def test_end_view_pattern_is_recorded_without_inventing_hole_depth(monkeypatch):
    ...
    async def fake_ask(prompt, *_args, **_kwargs):
        if "ТОЛЬКО мелкие элементы" in prompt:
            return {"chamfers": [{"size_mm": 1, "angle_deg": 45, "location": "left_end"}]}
        return {}
...
async def test_opposite_to_bore_holes_get_geometry_derived_depth(monkeypatch):
    ...
    evidence = {
        ...
        "blockers": ["radial-opening-3: контур допускает несколько диаметров Ø9/Ø10"],
    }
    monkeypatch.setattr(
        "app.ai.cad_recognize.turned_features.localize_turned_features",
        lambda *_args, **_kwargs: evidence,
    ...
    assert [
        (item["diameter_mm"], item["angle_deg"], item["depth_mm"]) for item in result["cross_holes"]
    ...
    assert not any("несколько диаметров" in item for item in profile_evidence["feature_unresolved"])
...
async def test_external_thread_carrier_requires_exact_chain_bounds(monkeypatch):
    ...
    assert [(item["diameter_mm"], item["length_mm"]) for item in outer] == [
        (102, 14),
        (80, 357),
        (72, 6.0),
        (75.0, 18.0),
