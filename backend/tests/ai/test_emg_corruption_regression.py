...
def test_class_balanced_safety_claims_are_backed_by_corruption_report():
    ...
    safety_report = run_emg_admission_corruption(
        emg_manifest, fixture_root=FIXTURES
    )

    report = evaluate_class_balanced_manifest(
        pipeline_manifest, safety_report=safety_report
    )

    assert report["by_class"]["rotation_body"]["safety_coverage_rate"] == 1.0
    assert report["by_class"]["assembly"]["critical_miss_rate"] == 0.0
    assert report["by_class"]["architectural_bim"][
