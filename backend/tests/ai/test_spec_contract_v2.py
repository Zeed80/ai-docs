...
def test_a_thread_annotates_the_step_it_sits_on():
    thread = SpecThread(
        designation="M75x1,5", nominal_diameter_mm=75.0, pitch_mm=1.5
    )
...
def test_a_groove_states_its_depth_or_its_root_never_both():
    ...
    with pytest.raises(ValidationError):
        SpecGroove(
            axial_position_mm=40.0, width_mm=3.0, depth_mm=2.0, root_diameter_mm=76.0
        )
    with pytest.raises(ValidationError):
...
def test_features_must_fit_inside_the_part():
    ...
    with pytest.raises(ValidationError):  # runs past the 470 mm part
        SpecBody.model_validate({**_SHAFT, "keyways": [
    ...
    with pytest.raises(ValidationError):  # a groove off the end of the part
        SpecBody.model_validate({**_SHAFT, "grooves": [
            {"axial_position_mm": 900.0, "width_mm": 3.0, "depth_mm": 2.0}
        ]})
    with pytest.raises(ValidationError):  # a cross hole nowhere near the part
        SpecBody.model_validate({**_SHAFT, "cross_holes": [
            {"diameter_mm": 14.0, "axial_position_mm": -5.0}
        ]})
...
def test_a_keyway_is_a_slot_not_a_point():
    with pytest.raises(ValidationError):
        SpecKeyway(
            axial_start_mm=10.0, length_mm=5.0, width_mm=12.0, depth_mm=6.0
        )
...
def test_the_spindle_can_finally_be_described_in_full():
    ...
    spec = EngineeringDrawingSpec.model_validate({
        ...
        "main_view": {
            **_SHAFT,
            "outer": [
                {"diameter_mm": 102.0, "length_mm": 150.0,
                 "taper": {"kind": "ratio", "ratio": "7:24"}},
                {"diameter_mm": 80.0, "length_mm": 200.0, "tolerance": "js6"},
                {"diameter_mm": 75.0, "length_mm": 120.0,
                 "thread": {"designation": "M75x1,5", "nominal_diameter_mm": 75.0,
                            "pitch_mm": 1.5}},
            ],
            "keyways": [
                {"axial_start_mm": 40.0, "length_mm": 85.0, "width_mm": 12.0,
                 "depth_mm": 5.0, "standard_ref": "ГОСТ 23360"},
            ],
            "grooves": [
                {"kind": "thread_runout", "axial_position_mm": 400.0,
                 "width_mm": 3.0, "depth_mm": 1.5},
            ],
            "cross_holes": [
