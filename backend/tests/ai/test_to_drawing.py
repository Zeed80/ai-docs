...
async def test_promote_ir_hole_gets_contour_and_diameter_dimension(db_session):
    ...
    contour = (
        await db_session.execute(
            sa.select(FeatureContour).where(FeatureContour.feature_id == hole.id)
        )
    ).scalar_one()
    ...
    dim = (
        await db_session.execute(
            sa.select(FeatureDimension).where(FeatureDimension.feature_id == hole.id)
        )
    ).scalar_one()
...
async def test_promote_ir_does_not_cross_attribute_thread_to_the_wrong_hole(db_session):
    ...
    ir = CadIR(
        ...
        entities=[
            ...
            Circle(
                center=Point(x=140, y=100), radius=10, confidence=0.9
            ),  # farther, but still in ITS OWN threshold
