...
from app.db.models import Document, DocumentStatus, InventoryItem, Invoice, InvoiceStatus
...
async def test_update_inventory_item(client: AsyncClient, bolt_item):
    resp = await client.patch(
        f"/api/warehouse/inventory/{bolt_item.id}",
        json={
...
async def test_issue_stock(client: AsyncClient, bolt_item):
    resp = await client.post(
        f"/api/warehouse/inventory/{bolt_item.id}/issue",
        json={
            "quantity": 50.0,
...
async def test_adjust_stock_positive(client: AsyncClient, bolt_item):
    resp = await client.post(
        f"/api/warehouse/inventory/{bolt_item.id}/adjust",
        json={
            "quantity": 100.0,
            "reason": "Поступление",
        },
