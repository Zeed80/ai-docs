...
def test_valid_fit_system_passes():
    ...
    features = [
        _feature(dimensions=[{"dim_type": "diameter", "nominal": 20.0, "fit_system": "H7"}])
    ]
    report = validate_drawing_extraction(uuid.uuid4(), features)
...
def test_fit_with_pair_passes():
    ...
    features = [
        _feature(dimensions=[{"dim_type": "diameter", "nominal": 20.0, "fit_system": "H7/k6"}])
    ]
    report = validate_drawing_extraction(uuid.uuid4(), features)
    assert report.tolerance_valid is True
...
def test_invalid_fit_warns():
    ...
    features = [
        _feature(
            dimensions=[{"dim_type": "diameter", "nominal": 20.0, "fit_system": "XYZ_invalid"}]
        )
    ]
    report = validate_drawing_extraction(uuid.uuid4(), features)
