    ...
    session = _session(
        [
            {"role": "user", "content": "Отправь поставщику письмо с просьбой прислать счёт"},
            {
                "role": "assistant",
                "content": (
                    "Подготовила письмо на sales@romex.example, тема «Запрос счёта». Отправляю?"
                ),
            },
        ]
    )
...
def test_capability_args_digest_matches_on_both_sides():
    ...
    assert capability_args_digest(args) != capability_args_digest({**args, "draft_id": "d2"})
...
async def test_a_personal_mailbox_is_never_the_default_sender(db_session):
    ...
    db_session.add(
        MailboxConfig(
            ...
            imap_host="mail.example.com",
            imap_port=993,
            imap_user="ceo@example.com",
            imap_password_encrypted="x",
            imap_ssl=True,
            smtp_host="smtp.example.com",
            smtp_port=587,
            smtp_user="ceo@example.com",
...
async def test_fetch_reports_only_mailboxes_the_caller_can_see(client, db_session, monkeypatch):
    ...
    db_session.add_all(
        [
            ...
            MailboxConfig(
                ...
                imap_host="mail.example.com",
                imap_port=993,
                imap_user="colleague@example.com",
                imap_password_encrypted="x",
                imap_ssl=True,
                is_active=True,
                mailbox_type="personal",
...
async def test_fetch_of_someone_elses_personal_mailbox_is_refused(client, db_session):
    ...
    db_session.add(
        MailboxConfig(
            ...
            imap_host="mail.example.com",
            imap_port=993,
            imap_user="private@example.com",
            imap_password_encrypted="x",
            imap_ssl=True,
            is_active=True,
            mailbox_type="personal",
            owner_sub="somebody-else",
        )
    )
    await db_session.commit()
...
async def test_reply_draft_into_a_colleagues_personal_thread_is_refused(client, db_session):
    ...
    db_session.add(
        MailboxConfig(
            name="colleague2@example.com",
            imap_host="mail.example.com",
            imap_port=993,
            imap_user="colleague2@example.com",
            imap_password_encrypted="x",
            imap_ssl=True,
            is_active=True,
            mailbox_type="personal",
    ...
    thread = EmailThread(subject="Личное", mailbox="colleague2@example.com", message_count=1)
    db_session.add(
        EmailMessage(
            thread=thread,
            mailbox="colleague2@example.com",
            subject="Личное",
            from_address="doctor@clinic.example",
            to_addresses=["colleague2@example.com"],
            body_text="результаты анализов",
