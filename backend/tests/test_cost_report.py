...
def telemetry(monkeypatch):
    def _set(rows):
        monkeypatch.setattr(
            "app.ai.telemetry.get_summary", lambda: {"by_model": rows}
        )
...
async def test_one_model_used_by_several_tasks_is_summed_once(telemetry):
    ...
    telemetry([
        {"task": "email_drafting", "model": "claude_sonnet_anthropic",
         "calls": 5, "tokens_in": 50_000, "tokens_out": 0},
        {"task": "tool_calling", "model": "claude_sonnet_anthropic",
         "calls": 3, "tokens_in": 50_000, "tokens_out": 0},
    ])
    report = await p.cost_report()
...
async def test_unknown_price_is_never_reported_as_zero(telemetry):
    telemetry([
        {"task": "email_drafting", "model": "нет_такой_модели_в_каталоге",
         "calls": 12, "tokens_in": 9_000, "tokens_out": 1_000}
    ])
    report = await p.cost_report()
...
async def test_local_models_are_not_listed_as_unpriced(telemetry):
    ...
    telemetry([
        {"task": "embedding", "model": "qwen3_embedding_4b_ollama",
         "calls": 1000, "tokens_in": 500_000, "tokens_out": 0}
    ])
    report = await p.cost_report()
