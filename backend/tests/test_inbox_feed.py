...
async def feed_data(db_session):
    ...
    db_session.add_all([
        ...
        MailboxConfig(
            name="colleague@example.com", display_name="Личный",
            imap_host="m.example.com", imap_port=993, imap_user="colleague",
            imap_password_encrypted="x", imap_ssl=True, is_active=True,
            mailbox_type="personal", owner_sub=OTHER_SUB,
        ),
    ])
    db_session.add_all([
        EmailThread(subject="Непрочитанное письмо", mailbox="procurement",
                    message_count=1, is_read=False, folder="inbox",
                    last_message_at=now - timedelta(minutes=5)),
        EmailThread(subject="Уже прочитанное", mailbox="procurement",
                    message_count=1, is_read=True, folder="inbox",
                    last_message_at=now - timedelta(minutes=4)),
        EmailThread(subject="Личное письмо коллеги", mailbox="colleague@example.com",
                    message_count=1, is_read=False, folder="inbox",
    ...
    db_session.add(AnomalyCard(
        ...
        status=AnomalyStatus.open, entity_type="invoice", entity_id=uuid.uuid4(),
...
async def test_a_colleagues_private_mail_never_enters_the_feed(
    client: AsyncClient, feed_data
):
    titles = [i["title"] for i in (await client.get("/api/inbox")).json()["items"]]
