...
def test_evaluate_conditions_all_any_regex():
    ...
    cond_regex = {"match": "any", "rules": [{"field": "subject", "op": "matches_regex", "value": r"№\s*\d+"}]}
    assert evaluate_conditions(msg, [], cond_regex, known_supplier_domains=set()) is True

    cond_supplier = {"match": "all", "rules": [{"field": "is_from_known_supplier", "op": "is_true"}]}
    assert evaluate_conditions(msg, [], cond_supplier, known_supplier_domains={"acme-supply.ru"}) is True
...
def test_apply_rules_adds_label_and_stops(sync_db):
    ...
    with Session(sync_db) as db:
        ...
        db.add(label)
        db.flush()
        th = EmailThread(subject="Счёт", mailbox="procurement", message_count=1,
                         last_message_at=datetime.now(timezone.utc))
        db.add(th)
        db.flush()
        msg = EmailMessage(
            thread_id=th.id, mailbox="procurement", from_address="sales@acme.ru",
            subject="Счёт на оплату №7", body_text="текст", is_inbound=True,
            received_at=datetime.now(timezone.utc),
            message_id_header=f"<{uuid.uuid4()}@acme.ru>",
        )
        db.add(msg)
        db.add(EmailRule(
            name="Счета от acme", mailbox=None, owner_sub=None, is_active=True, priority=10,
            stop_processing=True,
            conditions={"match": "all", "rules": [
                {"field": "from", "op": "contains", "value": "acme.ru"},
                {"field": "subject", "op": "contains", "value": "счёт"},
            ]},
            actions=[{"type": "add_label", "label_id": str(label.id)},
                     {"type": "mark_read"}],
        ))
        db.add(EmailRule(
            name="never runs", mailbox=None, owner_sub=None, is_active=True, priority=20,
            conditions={"match": "all", "rules": [{"field": "subject", "op": "contains", "value": "zzz"}]},
            actions=[{"type": "star"}],
        ))
        db.commit()
        ...
        link = db.query(EmailThreadLabel).filter_by(thread_id=th.id, label_id=label.id).one_or_none()
...
async def test_rules_crud_and_dry_run(client: AsyncClient, db_session):
    db_session.add(MailboxConfig(
        name="procurement", imap_host="h", imap_port=993, imap_user="p",
        imap_password_encrypted="x", imap_ssl=True, is_active=True, mailbox_type="shared",
    ))
    th = EmailThread(subject="Рекламация по браку", mailbox="procurement", message_count=1,
                     last_message_at=datetime.now(timezone.utc))
    db_session.add(th)
    await db_session.flush()
    db_session.add(EmailMessage(
        thread_id=th.id, mailbox="procurement", from_address="qa@partner.ru",
        subject="Рекламация по браку партии", body_text="брак", is_inbound=True,
        received_at=datetime.now(timezone.utc), message_id_header=f"<{uuid.uuid4()}@p.ru>",
    ))
    await db_session.commit()

    r = await client.post("/api/email/rules", json={
        "name": "Рекламации", "mailbox": "procurement",
        "conditions": {"match": "all", "rules": [{"field": "subject", "op": "contains", "value": "рекламац"}]},
        "actions": [{"type": "star"}],
    })
