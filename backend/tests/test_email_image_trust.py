...
async def test_trust_is_idempotent_and_revocable(client, db_session):
    ...
    for _ in range(2):
        resp = await client.post(
            "/api/email/contacts/trust-images",
            json={"email": "repeat@known.example"},
        )
        assert resp.status_code == 200, resp.text
...
async def test_always_show_images_covers_every_sender(client, db_session):
    ...
    if not (
        await db_session.execute(__import__("sqlalchemy").select(User).where(User.sub == me["sub"]))
    ).scalar_one_or_none():
        db_session.add(
            User(
                sub=me["sub"],
                email=me.get("email") or "t@example.com",
                name=me.get("name") or "Тест",
