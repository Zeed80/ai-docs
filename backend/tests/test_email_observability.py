    ...
    try:
        with Session(engine) as db:
            db.add(MailboxConfig(
                name="metricbox", display_name="M", imap_host="m.example.com",
                imap_port=993, imap_user="metricbox", imap_password_encrypted="x",
                imap_ssl=True, is_active=True,
            ))
            db.add(EmailRule(
                name="Пометить", mailbox="metricbox", owner_sub=None, is_active=True,
                priority=10,
            ...
            msg = EmailMessage(
                thread=thread, mailbox="metricbox", subject="Счёт на оплату",
                from_address="x@y.example", to_addresses=["metricbox@example.com"],
...
async def test_correcting_a_category_moves_the_quality_metric(client, db_session):
    ...
    from app.db.models import (
        EmailMessage, EmailThread, EmailTriageResult, MailboxConfig,
    )

    db_session.add(MailboxConfig(
        name="qual", display_name="Q", imap_host="m.example.com", imap_port=993,
        imap_user="qual", imap_password_encrypted="x", imap_ssl=True, is_active=True,
    ))
    thread = EmailThread(subject="Письмо", mailbox="qual", message_count=1)
    msg = EmailMessage(
        thread=thread, mailbox="qual", subject="Письмо", from_address="a@b.example",
        to_addresses=["qual@example.com"], received_at=datetime.now(timezone.utc),
        message_id_header=f"<{uuid.uuid4()}@b.example>",
    )
    db_session.add_all([thread, msg])
    await db_session.flush()
    db_session.add(EmailTriageResult(
        message_id=msg.id, mailbox="qual", category="newsletter", status="done",
    ))
    await db_session.commit()

    before = _sample(email_triage_corrections_total, was="newsletter", now="invoice")
    resp = await client.post(
        f"/api/email/messages/{msg.id}/triage/correct", json={"category": "invoice"},
    )
    assert resp.status_code == 200, resp.text
    assert _sample(
        email_triage_corrections_total, was="newsletter", now="invoice"
    ) == before + 1
...
async def test_health_screen_surfaces_what_is_quietly_broken(client, db_session):
    ...
    they become visible."""
    from app.db.models import (
        EmailAttachment, EmailMessage, EmailSyncOp, EmailThread, MailboxConfig,
        MailboxFolder,
    )

    db_session.add(MailboxConfig(
        name="healthbox", display_name="Здоровье", imap_host="m.example.com",
        imap_port=993, imap_user="healthbox", imap_password_encrypted="x",
        imap_ssl=True, is_active=True, sync_error="IMAP: connection refused",
    ))
    db_session.add(MailboxFolder(
        mailbox="healthbox", remote_name="INBOX", local_folder="inbox",
        sync_enabled=True, uid_validity=777, sync_error=None,
    ))
    thread = EmailThread(subject="Письмо", mailbox="healthbox", message_count=1,
                         is_read=False)
    msg = EmailMessage(
        thread=thread, mailbox="healthbox", subject="Письмо",
        from_address="a@b.example", to_addresses=["healthbox@example.com"],
    ...
    db_session.add(EmailAttachment(
        message_id=msg.id, filename="потерянное.pdf", content_type="application/pdf",
        size=10, storage_path=None, sha256="f" * 64,
    ))
    db_session.add_all([
...
async def test_mailbox_counts_are_cached_and_invalidated(client, db_session):
    ...
    db_session.add(MailboxConfig(
        name="countbox", display_name="Счётчики", imap_host="m.example.com",
        imap_port=993, imap_user="countbox", imap_password_encrypted="x",
        imap_ssl=True, is_active=True,
    ))
    thread = EmailThread(subject="T", mailbox="countbox", message_count=1,
                         is_read=False, folder="inbox")
    db_session.add(thread)
    await db_session.flush()
    db_session.add(EmailMessage(
        thread_id=thread.id, mailbox="countbox", subject="T",
        from_address="a@b.example", to_addresses=["countbox@example.com"],
        received_at=datetime.now(timezone.utc),
        message_id_header=f"<{uuid.uuid4()}@b.example>",
    ))
    await db_session.commit()
    ...
    await invalidate_mailbox_counts()
    assert (await mailbox_counts(db_session, ["countbox"])).for_mailbox(
        "countbox")["unread"] == 0
...
async def test_body_retention_removes_content_but_keeps_the_record(db_session):
    ...
    for name in ("shortbox", "foreverbox"):
        ...
        await db_session.flush()
        msg = EmailMessage(
            thread_id=thread.id, mailbox=name, subject="Старое письмо",
            from_address="a@b.example", to_addresses=[f"{name}@example.com"],
            body_text="Коммерческая тайна", body_html="<p>Коммерческая тайна</p>",
            snippet="Коммерческая тайна", received_at=old,
            message_id_header=f"<{uuid.uuid4()}@b.example>",
        )
        db_session.add(msg)
        await db_session.flush()
