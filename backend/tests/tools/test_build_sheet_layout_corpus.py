...
MODULE_PATH = (
    Path(__file__).resolve().parents[3] / "tools" / "cad-dataset" / "build_sheet_layout_corpus.py"
)
...
def test_layout_corpus_preserves_source_group_split(tmp_path: Path) -> None:
    ...
    (source / "manifest.jsonl").write_text("".join(json.dumps(row) + "\n" for row in rows))
    ...
    assert {row["split"] for row in output if row["source_group_id"] == "part-a"} == {"train"}
    assert {row["split"] for row in output if row["source_group_id"] == "part-b"} == {"holdout"}
