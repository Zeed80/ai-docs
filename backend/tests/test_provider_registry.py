...
# ── reasoning-effort level resolution ───────────────────────────────────────


def _make_level_capable(
    router: AIRouter,
    model_key: str,
    *,
    default: str = "high",
    ...
    """Mutate a catalog entry in-place to declare level support for a test."""
    cap = router.registry.models[model_key]
    router.registry.models[model_key] = cap.model_copy(
        update={
            "thinking_supported": True,
            "thinking_enabled": True,
            "thinking_levels": levels or ["low", "medium", "high"],
            "thinking_level_default": default,
        }
    )


@pytest.mark.asyncio
async def test_thinking_level_defaults_to_model_catalog(monkeypatch, router_with_stub):
    r, stub = router_with_stub
    _route_to(monkeypatch, "gemma4_e4b_ollama")
...
@pytest.mark.asyncio
async def test_thinking_level_per_call_override_wins(monkeypatch, router_with_stub):
    r, stub = router_with_stub
    _route_to(monkeypatch, "gemma4_e4b_ollama")
    _make_level_capable(r, "gemma4_e4b_ollama", default="high")
    await r.run(
...
@pytest.mark.asyncio
async def test_thinking_level_ignored_when_thinking_off(monkeypatch, router_with_stub):
    r, stub = router_with_stub
    _route_to(monkeypatch, "gemma4_e4b_ollama")
    _make_level_capable(r, "gemma4_e4b_ollama", default="high")
    await r.run(
    ...
    assert stub.last_request.thinking is False
    assert stub.last_request.thinking_level is None


@pytest.mark.asyncio
async def test_thinking_level_clamped_to_supported(monkeypatch, router_with_stub):
    r, stub = router_with_stub
    _route_to(monkeypatch, "gemma4_e4b_ollama")
    # Model only declares low/medium (no "high") — a per-call request for a
    # level the model doesn't support must clamp to the first declared
    # level, not raise or silently pass the unsupported value through.
