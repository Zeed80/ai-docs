...
async def test_sheet_create_edit_formula_lifecycle(client):
    ...
    await client.post(
        f"/api/workspace/sheets/{sheet_id}/patch-cells",
        json={
            "edits": [{"row": 0, "col": "quantity", "value": 5}],
        },
    )
    await client.post(
        f"/api/workspace/sheets/{sheet_id}/add-row",
        json={
    ...
    await client.post(
        f"/api/workspace/sheets/{sheet_id}/add-column",
        json={
            "key": "vat",
            "header": "НДС",
            "type": "number",
...
async def test_sheet_from_spec_is_editable_copy(client):
    ...
    resp = await client.post(
        "/api/workspace/sheets/from-spec",
        json={
            "canvas_id": "agent:spec-table",
...
async def test_sheet_from_template_estimate(client):
    ...
    resp = await client.post(
        "/api/workspace/sheets/from-template",
        json={
            "template": "смета",
        },
    )
...
async def test_rename_column(client):
    ...
    sheet_id = resp.json()["sheet_id"]
    r = await client.post(
        f"/api/workspace/sheets/{sheet_id}/rename-column",
        json={
            "key": "A",
            "header": "Поставщик",
        },
...
async def test_sheet_merge_unmerge_cells(client):
    ...
    merged = await client.post(
        f"/api/workspace/sheets/{sheet_id}/merge-cells",
        json={
            "start_row": 0,
            "end_row": 0,
            "start_col": "A",
    ...
    unmerged = await client.post(
        f"/api/workspace/sheets/{sheet_id}/unmerge-cells",
        json={
            "row": 0,
            "col": "A",
        },
