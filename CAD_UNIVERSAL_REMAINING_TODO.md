...
- [-] Снять class-balanced baseline reader → graph → 3D/BIM → 2D: baseline `class_balanced_dev_baseline_20260814_mechanical.json` фиксирует все четыре стадии без подмены `not_run` успехом. Graph покрыт `4/4`; drawing coverage/pass подняты `0,25/0 → 1,0/1,0`: assembly/construction/system проходят deterministic SVG reopen + GraphPatch verification, а exact `mechanical-detal-126-v2` — live FreeCAD STEP reopen и непустые front/side/top TechDraw projections. Mechanical model 3D также прошёл, поэтому macro 3D/BIM pass `0 → 0,25`; assembly/construction build evidence и reader всё ещё неполны, hydraulic 3D неприменим, поэтому `promotion_eligible=false`.
