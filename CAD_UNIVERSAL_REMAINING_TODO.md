...
- [-] Принимать mechanical input только как полный валидный `EngineeringDrawingGraph`: assembly STEP и legacy mechanical B-Rep теперь оба проходят общий fail-closed admission непосредственно перед kernel call; legacy raster reader пока совместимо проецирует проверенный spec/feature tree в sealed EMG, а не всегда начинает с полного `EngineeringDrawingGraph`. Его evidence/dimension/spec gates сохранены и объединены с EMG blockers; tampered hash гарантированно останавливает kernel.
...
- [-] Валидировать единицы, масштаб, систему координат, обязательные виды и противоречия размеров: общий admission fail-closed потребляет результаты 12-level verifier для единиц, coordinate system, required 2D и validated contradictions. Metric `drawing.scale_mm_per_px` проверяется на положительное число; legacy title-block scale — на точную ГОСТ-серию без угадывания; отсутствие локализованного scale evidence явно помечается `drawing_scale_evidence_missing`. Осталось связать scale evidence с каждым релевантным view/detail, а не только листом.
...
- [x] Side-by-side связь entity ↔ bbox/page ↔ view/topology/BIM object: выбранная immutable operation/Feature раскрывается в assertion, точный source crop/лист/bbox и серверный target-specific dependency impact до build operations, topology IDs, artifacts, связанных `View` и BIM/IFC objects. Для view/BIM показывается полный dependency path; BIM-классификация разрешена только для construction/MEP-профилей либо узлов с явными `ifc.*`/`bim.*`/`spatial.*` assertions. Если связь отсутствует, UI сообщает это явно и ничего не додумывает.
...
- [x] Показать блокеры, альтернативы и влияние каждой неопределённости на 3D/виды: CAD-редактор имеет общий список critical unresolved assertions, открывает точное утверждение и связанную операцию только через явную subject/realizes связь либо серверный target-specific dependency closure. Для связанного `HypothesisSet` показывает все сохранённые `HypothesisOption` с выбранным вариантом, assertion values, hard-constraint status, evidence coverage, cross-view consistency и assumption count; отсутствующую связь не додумывает. Provenance-инспектор затем показывает bbox и влияние на operations/topology/artifacts/views/BIM.
...
- [x] После правки пересчитывать только зависимые узлы и сохранять audit diff/author/time: immutable audit diff/actor/time и точный dependency closure выполнены. CAD-kernel переиспользует как неизменённые независимые `body_index`, так и самый длинный точный префикс фактической последовательности операций внутри тела. Каждый bounded-LRU checkpoint имеет chain SHA по предыдущему checkpoint, stage/ordinal/global feature index и полному feature payload; detached BREP перед hit заново открывается и проходит `isValid`/`isClosed`/volume/topology-signature gates. После каждой операции сохраняются topology signature, инварианты и компактные added/removed face/edge deltas. Изменение порядка, индекса, параметров, `body_index` или assumptions mode инвалидирует изменённый и все последующие checkpoints. Финальные STEP/IGES/STL/topology и общий STEP reopen всегда выпускаются и проверяются заново.
...
- [x] Добавить download всех диагностических артефактов одним пакетом: owner-scoped `diagnostics-package` доступен и для blocked/review drafts, детерминированно упаковывает source, generation/workflow snapshot, текущие CAD IR/EMG, GraphPatch journal и имеющиеся 2D/3D/topology artifacts. `manifest.json` фиксирует размер и SHA-256 каждого файла, а обязательные отсутствующие/недоступные артефакты перечисляет в `missing_artifacts` и выставляет `complete=false`; storage failures не замалчиваются. ZIP доступен из меню редактора до приёмки, тогда как release CAD-экспорт остаётся approval-gated.

## 8. Итерационный цикл качества

- [x] Сформировать минимальное число source groups на каждый поддержанный promotion-класс: pre-holdout dev candidate содержит четыре независимые source groups (`rotation_body`, `assembly`, `architectural_bim`, `hydraulic_system`) и явно фиксирует complexity каждого case. Это минимальный promotion smoke, а не заявление о полном покрытии всей долгосрочной матрицы классов из разделов 3–4.
- [x] Снять class-balanced baseline reader → graph → 3D/BIM → 2D: immutable candidate `class_balanced_dev_candidate_20260814.json` не засчитывает `not_run` и требует внешние evidence reports. Structured schema/import reader, canonical graph и 2D дали `1,0/1,0`; применимые 3D/BIM дали `1,0/1,0`. Exact mechanical проходит FreeCAD STEP reopen + front/side/top TechDraw; assembly — 2-solid STEP reopen; construction — IFC4 geometry reopen; hydraulic 3D имеет отдельный `not_applicable` reason и исключён из знаменателя. Safety coverage `1,0` по всем классам, promotion failures `0`.
...
- [x] Исправлять общий класс ошибки; запрещена подгонка по имени файла/fixture: stage/safety evidence сопоставляется по case ID, stage, source group и drawing class; STEP signed-zero исправлен общим DATA canonicalizer, а не условием по fixture.
- [x] После изменения прогонять unit + domain dev + corruption tests: единая команда `make cad-class-balanced-cycle` сначала проверяет пять fail-closed admission-corruptions, четыре deterministic graph golden cases и три independently reopened 2D artifacts, затем baseline delta и смежные unit/domain regressions.
