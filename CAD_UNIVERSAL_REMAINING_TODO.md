...
- [-] Дать безопасное редактирование параметров и связей с повторной локальной валидацией: immutable GraphPatch/rebuild и impact preview выполнены, локальная проверка только затронутого подграфа ещё не заменяет полную пересборку.
