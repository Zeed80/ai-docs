...
- [-] Добавить фильтры ошибок, навигацию клавиатурой, масштабирование и удобный выбор мелкой геометрии: дерево теперь фильтруется по операциям с предположениями/без них, поддерживает `ArrowUp`/`ArrowDown`/`Home`/`End`, а 3D-вьюпорт — явные zoom/fit controls и точное приближение к kernel bbox выбранной операции. Остались фильтры по доменным validation/error codes и отдельный список мелких face/edge topology IDs для выбора без попадания мышью.
