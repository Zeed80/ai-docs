...
- [-] Снять class-balanced baseline reader → graph → 3D/BIM → 2D: baseline `class_balanced_dev_baseline_20260814.json` фиксирует все четыре стадии без подмены `not_run` успехом. Graph покрыт `4/4` golden evidence; deterministic 2D artifact regression с повторным SVG-open и GraphPatch verification поднял drawing coverage `0,25 → 0,75` и pass rate `0 → 0,75` (`assembly`, `architectural_bim`, `hydraulic_system`) относительно baseline 2026-08-13. Mechanical 2D, reader и 3D/BIM остаются неполными, поэтому `promotion_eligible=false`.
