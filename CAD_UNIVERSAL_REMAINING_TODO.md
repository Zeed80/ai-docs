...
- [-] После правки пересчитывать только зависимые узлы и сохранять audit diff/author/time: immutable audit diff/actor/time и вычисление точного dependency closure выполнены; kernel пока пересобирает цель целиком, инкрементальное переиспользование незатронутых B-Rep/artifacts остаётся отдельной задачей.
