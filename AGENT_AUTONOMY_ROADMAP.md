...
- [x] Прогнать полный цикл: создание → decompose по поставщикам → `web_discover` через `ComputerUseGrant` → `tool_catalog.create_supplier`/`ingest_web_source` → replanning нескольких горизонтов (revision 1→7, все находки выше обнаружены и исправлены именно в процессе этих реплан-циклов) → honest-coverage критерии → терминальный статус. **Итог: `blocked`, не `completed`** — и это ЗАКОНОМЕРНЫЙ, а не аварийный исход: независимый semantic verifier (критерий `honest_not_found`) корректно отказался засчитать `synthesize`-шаг, чей вывод был обрывком незавершённого исследования («Страница YG1 содержит только общее описание... Попробую получить каталог с конкретными артикулами:»), а не структурированным `{covered, partial, not_found}` отчётом. Агент реально нашёл и завёл поставщика Haltec (create_supplier + ingest_web_source), поставщик Betar упал из-за инфраструктурной находки выше (permanently failed после lease_expired), нашёл третьего (YG1), но не уложился в бюджет на извлечение его каталога и не оформил честную сводку по нему до исчерпания `max_replans`. Fail-closed дизайн Ф1.D сработал ровно так, как задумано: система НЕ подделала «успех» там, где реального завершения не было.
