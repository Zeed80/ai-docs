...
- [ ] Повторить живой exploratory-пилот (тот же или похожий объектив — каталоги поставщиков) на версии с этим фиксом и подтвердить: либо реальное `completed` на большем числе поставщиков за счёт персистентности, либо, если снова `blocked`/бюджет исчерпан, — что `not_found`-пункты в отчёте отражают честно МНОЖЕСТВЕННЫЕ разные попытки (не одну), а не то же самое раннее отступление под новой оберткой.
...
- [ ] Ручная сквозная проверка на 3 разных типах источников (прямой PDF-прайс / HTML-каталог / защищённый капчей) — пилот использовал реальный открытый web-поиск без предзаданных URL (агент сам выбирал источники), что честно проверило `web_discover`+`ComputerUseGrant`, но не покрыло целенаправленно каждый из трёх типов защиты по отдельности. Итоговый honest-coverage отчёт получен как обрывок (см. блокер выше), не как эталонный завершённый отчёт — непригоден как regression-фикстура в текущем виде. Оставлено как открытый TODO для отдельного целевого прогона (не блокирует закрытие Ф4: сама МЕХАНИКА end-to-end цикла — decompose, web_discover, catalog ingestion, replanning, honest fail-closed verification — подтверждена живьём).
...
- [ ] Ручная проверка «exploratory WorkOrder дважды на одном домене → второй прогон создаёт draft-connector, третий использует его» — НЕ прогонялась как отдельный живой end-to-end пилот (второй полноценный пилот после Ф4 не запускался в рамках Ф5 — время/объём одного пилота уже показал масштаб в Ф4). Механика подтверждена на уровне интеграционных HTTP-тестов с реальной БД/Qdrant (см. выше), но не сквозным пилотом на реальных сайтах. Открытый TODO, не выдаётся за подтверждённое.
