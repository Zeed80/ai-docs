...
> Статус: экспериментальный проект, находится на стадии активной разработки и пока не является работоспособным продуктом для реального использования.
...
- **Постобработка — opt-in**: по умолчанию отдаётся сырой результат ComfyUI (без бинаризации/векторизации и OCR-вставки текста, которые на реальных плотных чертежах добавляли артефакты); улучшения выбираются в UI (Авто/Нет/Лёгкая/Полная), LoRA-воркфлоу сохраняют свой настроенный режим.
...
- **Пресеты почтовых провайдеров** (Gmail, Outlook/Microsoft 365, Яндекс.Почта, Mail.ru, iCloud, свой сервер) с автозаполнением IMAP/SMTP-параметров и подсказкой по настройке; **вход через OAuth2** для Gmail, Microsoft 365, Яндекса и Mail.ru (без пароля вообще, отзываемо в любой момент) как альтернатива паролю приложения — см. [раздел ниже](#внешние-почтовые-ящики-gmail--яндекс--mailru--outlook).
- Собственный self-hosted почтовый сервер (Mailcow, см. [раздел ниже](#почтовый-сервер-mailcow)): личные `@домен`-ящики сотрудников (выдаёт админ), автоматическая классификация вложений тем же конвейером, что и общие ящики, ручная отправка вложения в документы/чертежи.
...
Собственный self-hosted почтовый сервер на домене компании — чтобы не зависеть
от внешних провайдеров (Яндекс/Gmail и т.п.). Ядро — [Mailcow](https://mailcow.email/)
(Postfix + Dovecot + Rspamd + ClamAV + SOGo webmail/календарь/контакты),
разворачивается отдельным compose-проектом рядом с основным стеком и
проксируется через уже работающий Traefik (HTTPS/Let's Encrypt).
...
2. Установка — из админки или из консоли.

   **Из админки**: **Администрирование → Интеграции → «Развернуть Mailcow»** —
   укажите хост и часовой пояс и нажмите кнопку. Бэкенд не может сам запускать
   `docker compose`, поэтому заявку исполняет host-агент
   (`infra/installer/update-agent.sh`, systemd-таймер — см.
   `infra/installer/update-agent.README`); прогресс и лог видны в той же
   карточке. Пошаговое руководство по ручной части (DNS, фаервол, DKIM,
   API-ключ) — **Интеграции → Настройка Mailcow**
   (`/admin/integrations/mailcow-guide`).
   ...
   **Из консоли** — то же самое:
   ...
   Скрипт клонирует `mailcow-dockerized` на пинованный релиз-тег (`MAILCOW_TAG`
   в `infra/.env`), генерирует `infra/mailcow/mailcow.conf` (веб-TLS терминирует
   наш Traefik, ClamAV/Solr выключены ради RAM), подключает вебмейл/admin/API к
   сети Traefik, перерендеривает роуты и **копирует сертификат Let's Encrypt в
   Mailcow** (`mailcow-certdump.sh`) — без этого шага Postfix/Dovecot отдают
   самоподписанный сертификат и почтовые клиенты не подключаются.

   После установки откройте фаервол для почтовых портов
   `25/465/587/143/993/110/995` (публикуются Mailcow напрямую на хосте, Traefik
   их не проксирует), создайте домен + API-ключ в Mailcow admin UI
   (`https://mail.example.com`, `Configuration → Access → Edit administrator
   details → API`, Read-Write) и **внесите IP контейнера backend в белый список
   ключа** — иначе валидный ключ отвечает 401/403. Включите таймер обновления
   сертификата: `mailcow-certdump.timer` (см. `infra/installer/mailcow.README`).

   Подробный чеклист и troubleshooting — `infra/installer/mailcow.README`.

3. Подключение к приложению: **Администрирование → Интеграции** → блок
   «Почтовый сервер (Mailcow)» — впишите API URL/ключ, домен, ссылку на
   вебмейл, IMAP/SMTP хосты и квоту ящика по умолчанию. Кнопка «Сохранить и
   проверить» сразу валидирует ключ; при 401/403 подсказывает про белый список
   IP.

### Личные почтовые ящики сотрудников

Ящики `@<домен>` выдаёт администратор (не self-service): на странице
пользователя **Администрирование → Пользователи → карточка → Корпоративная
почта** — кнопка «Создать почтовый ящик» (local-part предлагается
автоматически из имени, можно поменять). Пароль показывается один раз сразу
после создания — передайте его сотруднику. Там же — квота, сброс пароля,
переключатель сбора почты агентом и два разных способа отзыва: «Отключить
ящик» (обратимо, письма сохраняются) и «Удалить безвозвратно» (требует ввести
адрес полностью, уничтожает всю переписку).

Сотрудник видит свой адрес в **Настройки → Почта → Моя корпоративная почта**
(ссылка на вебмейл и на список своих писем в приложении). Любой обычный
почтовый клиент (Outlook, Thunderbird, Apple Mail, K-9 и т.п.) подключается
вводом только email + пароля — автонастройка через `autoconfig`/`autodiscover`
DNS-записи выше.

**Приватность личных ящиков.** Содержимое личного ящика видит только его
владелец: коллеги и администраторы получают 404/403 в списках, поиске,
spec-таблицах и деталях письма (`backend/app/domain/email_access.py`). Агент
(«Света») работает от имени того, с кем ведёт диалог (`X-Acting-User`,
`backend/app/auth/acting.py`) — то есть видит ровно то же, что и сам
собеседник; в headless-режимах (AgentCron, Telegram) личные ящики недоступны
вовсе. Администратор управляет ящиком (выдать, сбросить пароль, отозвать), но
не читает его. Общие ящики (закупки/бухгалтерия/общий) доступны всем как
раньше.

### Обработка почты

Общие ящики (закупки/бухгалтерия/общий) опрашиваются периодической задачей
триажа: вложения классифицируются и, если это счёт/документ, распознаются.

Личный ящик подключается к этому конвейеру **только по согласию владельца** —
галочка «Разрешить ИИ-сотруднику читать этот ящик» в **Настройки → Почта**
(по умолчанию выключена, отзывается в любой момент; админ видит и может
переключить её на карточке пользователя). Сбор идёт через `BODY.PEEK`, поэтому
отметки «прочитано» в почтовом клиенте сотрудника не сбиваются, а уведомления о
новых письмах уходят только владельцу. Если автоматика не сработала или нужен принудительный
прогон — на странице письма (**Почта → тред → вложение**) есть кнопки
«В документы» (OCR/извлечение) и «В чертёж» (CAD-векторизация/оцифровка).

### Обновление и бэкап

```bash
infra/installer/update-mailcow.sh --check        # текущий/целевой тег
infra/installer/update-mailcow.sh --yes           # бэкап → апдейт → health-check → авто-откат при сбое
```

Данные Mailcow уже включены в общий `infra/installer/backup.sh`/`restore.sh`.
Опциональный systemd-таймер еженедельно проверяет наличие обновлений (без
авто-применения) — `infra/installer/mailcow-update-check.timer/.service`.

## Внешние почтовые ящики (Gmail / Яндекс / Mail.ru / Outlook)

Кроме личных `@<домен>`-ящиков на своём Mailcow (см. выше), общий ящик отдела
(закупки / бухгалтерия / общий) может быть обычным внешним ящиком — своим на
Gmail, Яндексе, Mail.ru или Microsoft 365. Настраивается в **Настройки →
Почтовые ящики**.

### Пресеты провайдеров

Выпадающий список «Тип почты» при добавлении/редактировании ящика
автозаполняет IMAP/SMTP хост, порт и режим TLS и показывает подсказку именно
для выбранного провайдера — что включить в веб-версии почты и где взять
пароль приложения. Пресеты: Gmail, Outlook/Microsoft 365, Яндекс.Почта,
Mail.ru, iCloud Mail, «другой/корпоративный сервер» (`GET /api/mailbox/presets`).

### Аутентификация: OAuth2 или пароль приложения

Обычный пароль аккаунта у Gmail и (в большинстве организаций) у Microsoft 365
для IMAP/SMTP уже не работает. Два поддерживаемых способа:

- **OAuth2** — поддерживаются Google, Microsoft, Яндекс и Mail.ru;
  настраиваются независимо, можно включить любой набор. Администратор один
  раз регистрирует OAuth-приложение в **Администрирование → Интеграции →
  «OAuth2 для почты»**:
  1. создать OAuth-клиент в консоли провайдера — Google Cloud Console
     (`APIs & Services → Credentials`), Azure Portal (`App registrations`),
     `oauth.yandex.ru` (с правами на почту) или `oauth.mail.ru`;
  2. добавить в список разрешённых redirect URI значение, показанное в этой
     же карточке (`.../api/oauth/callback/google` или `/microsoft`);
  3. вставить Client ID/Secret и сохранить.

  Дальше — кнопка «Войти через …» в **Настройки → Почтовые ящики**:
  всплывающее окно согласия, и ящик подключён без единого пароля.
  Access-токен обновляется автоматически по refresh-токену (обновление
  сериализовано Redis-локом, поэтому параллельные приём и отправка не
  «сожгут» токен друг друга). Удаление ящика отзывает выданный доступ на
  стороне провайдера, а не просто забывает токен. Список ящиков показывает
  бейдж `OAuth2 ✓` либо `OAuth2 — не подключено`.

  Если доступ отозвали снаружи (в настройках аккаунта Google/Яндекса), сбор
  почты остановится с ошибкой авторизации в карточке ящика — восстанавливается
  кнопкой «Переподключить» в той же форме.
- **Пароль приложения** — второй вариант для Gmail (нужна двухфакторная
  аутентификация аккаунта и пароль на `myaccount.google.com/apppasswords`),
  Яндекса и Mail.ru, и единственный вариант для iCloud — точные шаги для
  каждого провайдера показаны в подсказке под выбранным пресетом.

Кнопка «Тест» у ящика проверяет и IMAP, и SMTP независимо от способа
аутентификации (XOAUTH2 для OAuth2-ящиков, обычный логин/пароль для
остальных).

**Права.** Общие ящики отдела — инфраструктура организации: их настройка
(включая привязку OAuth-аккаунта) доступна только роли `admin`, и у остальных
секция в настройках не показывается. Свой личный `@<домен>`-ящик сотрудник
подключает сам — на него прав админа не требуется.
