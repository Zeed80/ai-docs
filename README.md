...
- **Пресеты почтовых провайдеров** (Gmail, Outlook/Microsoft 365, Яндекс.Почта, Mail.ru, iCloud, свой сервер) с автозаполнением IMAP/SMTP-параметров и подсказкой по настройке; **вход через OAuth2** для Gmail/Microsoft 365 (без пароля вообще, отзываемо в любой момент) как альтернатива паролю приложения — см. [раздел ниже](#внешние-почтовые-ящики-gmail--яндекс--mailru--outlook).
...
- **OAuth2** (рекомендуется для Gmail/Microsoft 365, доступен для обоих
  провайдеров одновременно — выбор при подключении). Администратор один раз
  регистрирует OAuth-приложение в **Администрирование → Интеграции → «OAuth2
  для почты»**:
  1. создать OAuth-клиент в Google Cloud Console (`APIs & Services →
     Credentials`) или Azure Portal (`App registrations`);
  ...
  Дальше любой сотрудник в **Настройки → Почтовые ящики** жмёт «Войти через
  Google/Microsoft» — всплывающее окно согласия — и ящик подключён без
  единого пароля; access-токен обновляется автоматически по refresh-токену,
  доступ отзывается кнопкой «Переподключить» здесь же или прямо в аккаунте
  Google/Microsoft. Список ящиков показывает бейдж `OAuth2 ✓` либо `OAuth2 —
  не подключено`.
- **Пароль приложения** — второй вариант для Gmail (нужна двухфакторная
  аутентификация аккаунта и пароль на `myaccount.google.com/apppasswords`), а
  также единственный вариант для Яндекса, Mail.ru и iCloud — точные шаги для
