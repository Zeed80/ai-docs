...
2. Установка:
   ...
   `infra/mailcow/mailcow.conf` (TLS отключён — терминирует наш Traefik) и
...
3. Подключение к приложению: **Администрирование → Интеграции** → блок
   ...
   вебмейл, IMAP/SMTP хосты. «Проверить подключение» валидирует API-ключ.
...
после создания — передайте его сотруднику. Там же — сброс пароля и отзыв
ящика.
...
Личные ящики опрашиваются той же периодической задачей триажа, что и общие
(закупки/бухгалтерия/общий) — вложения автоматически классифицируются и,
если это счёт/документ, распознаются (тот же контент-based конвейер, без
отдельной настройки). Если автоматика не сработала или нужен принудительный
