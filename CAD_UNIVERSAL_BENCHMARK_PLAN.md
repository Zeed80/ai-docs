...
- [~] Добавить mechanical B-Rep evaluator и construction IFC evaluator. Mechanical pair evaluator проверяет B-Rep validity, topology, bounding box, volume/area, exact boolean volume IoU и двунаправленное surface sampling; живой STEP self-pair прошёл с IoU `1,0`, а production segfault в `Part.makeCompound` устранён безопасным single-shape/native Compound путём. Construction pair evaluator проверяет IFC validity, multiset классов, этажи/пространства, containment, geometry failures, axis-invariant bbox и агрегированный объём; self-pair прошёл, чужая модель отклонена четырьмя независимыми gates. Осталась orientation normalization механического B-Rep.
