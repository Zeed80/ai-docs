...
- [-] Снять PMI baseline production reader: на первом CTC-листе geometry schema reject сначала стирал все callouts (`0/10`). После разделения observation envelope и build gate чтение не потеряно, но качество остаётся неприемлемым: semantic F1 `0,071429`, missing `90%`, invented `94,44%`; полный прогон `30` листов остаётся впереди.
