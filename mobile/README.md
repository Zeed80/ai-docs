# Света — Android-оболочка (Capacitor)

Тонкая нативная оболочка, которая грузит **живой сайт** «Света» в WebView и добавляет возможности
...
- `capacitor.config.ts` — без `server.url`; `allowNavigation: ["*"]`, плагины.
...
- `android-overrides/` — файлы, накладываемые на сгенерированный `android/` (манифест, FileProvider,
  кастомные плагины Kotlin в пакете `ru.aiworkspace.sveta`).
...
- **AppUpdate** — самообновление: читает `<сервер>/download/version.json`, скачивает подписанный APK,
  проверяет sha256, запускает системный установщик через FileProvider.
- **SvetaPush** — владеет персональным ntfy-топиком (секрет) и foreground-службой подписки
  (`SvetaPushService`) — push без Google services. В payload только title/body/type/action_url.
...
`ru.aiworkspace.sveta`. `namespace`/`applicationId` (`ru.aiworkspace.sveta`) Capacitor берёт из
...
```bash
keytool -genkey -v -keystore sveta-release.jks -keyalg RSA -keysize 4096 \
  -validity 10000 -alias sveta
```
